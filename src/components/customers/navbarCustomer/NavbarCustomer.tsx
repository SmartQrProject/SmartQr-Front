...
const NavbarCustomer = ({ slug, name }: NavbarCustomerProps) => {
  ...
  // Fetch restaurant info when slug changes or on mount if slug exists in localStorage
  useEffect(() => {
    const fetchRestaurant = async () => {
      try {
        const targetSlug = slug || localSlug || localStorage.getItem('slug');
        if (!targetSlug) return;

        const res = await fetch(`/api/store/${targetSlug}`);
        if (!res.ok) {
          ...
          {isMenuOpen && (
            ...
            >
              <div>
                ...
                <button
                  onClick={() => {
                    console.log("clic en info store")
                    setIsMenuOpen(false);
                    setIsStoreInfoOpen(true);
                  }}
                  className="block py-2 px-3 text-gray-900 rounded-sm bg-amber-300 hover:bg-red-800"
                >
                  Store Info
                </button>
              </div>
