...
const TopClientesPorGasto = () => {
    ...
    useEffect(() => {
        ...
        const fetchData = async () => {
            try {
                const res = await fetch(`${APIURL}/${slug}/reports/customers?sortBy=totalSpent&order=desc&limit=10&page=1`, {
    ...
    return (
        <div className="bg-white p-4 sm:p-6 rounded-xl w-full mb-6">
            <h3 className="text-lg sm:text-xl font-bold mb-4 text-left sm:text-center">Top 10 Customers by Total Spending</h3>
                ...
                <div className="h-[500px]">
                    <ResponsiveContainer width="100%" height="100%">
                        <BarChart data={data} layout="vertical" margin={{ top: 0, right: 50, left: 100, bottom: 0 }}>
                            ...
                            <Tooltip formatter={(value: number, name: string) => (name === "totalSpent" ? [`$${value.toFixed(2)}`, "Total spent"] : [`${value}`, "Orders"])} />
