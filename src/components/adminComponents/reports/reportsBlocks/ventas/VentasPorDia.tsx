...
const VentasPorDia = ({ exportRef }: { exportRef?: React.MutableRefObject<any> }) => {
    const { user } = useAuth();
    ...
    useEffect(() => {
        const fetchData = async () => {
            ...
            try {
                const res = await fetch(`${APIURL}/${slug}/reports/sales?from=${today}&to=${today}`, {
    ...
    return (
        <div ref={chartRef} className="bg-white p-4 sm:p-6 rounded-xl w-full mb-6 max-w-full">
            <h3 className="text-lg sm:text-xl font-bold mb-4 text-center">Daily Sales</h3>
            ...
            {loading ? (
                ...
                <p className="text-sm text-gray-500 text-center">No sales recorded today.</p>
            ) : (
