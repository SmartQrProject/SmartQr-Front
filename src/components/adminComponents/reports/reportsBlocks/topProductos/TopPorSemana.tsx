...
const TopPorSemana = () => {
    ...
    useEffect(() => {
        ...
        const fetchData = async () => {
            try {
                const res = await fetch(`${APIURL}/${slug}/reports/topProducts?from=${start}&to=${end}&sort=${sort}`, {
    ...
    return (
        <div className="bg-white p-4 sm:p-6 rounded-xl w-full mb-6">
            <h3 className="text-lg sm:text-xl font-bold mb-4 text-left sm:text-center">Top products of the week</h3>
            ...
            {loading ? (
                ...
                <p className="text-sm text-gray-500 text-center">No products sold this week.</p>
            ) : (
