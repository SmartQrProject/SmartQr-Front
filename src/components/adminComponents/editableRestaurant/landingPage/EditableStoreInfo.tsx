...
const EditableStoreInfoModal = ({ restaurant, open, onClose }: StoreInfoModalProps) => {
  ...
  const [tags, setTags] = useState(
    restaurant.tags?.map(tag => tag.trim()).join(", ") || ""
  );
  const [description, setDescription] = useState(restaurant.description || "");
  const [phone, setPhone] = useState(restaurant.phone || "");

  const [showTradingHours, setShowTradingHours] = useState(
    !!restaurant.trading_hours
  );
  const [showOrderingTimes, setShowOrderingTimes] = useState(
    !!restaurant.ordering_times
  );
      ...
      <div className="relative w-full max-w-xl rounded-xl p-6 mx-4 bg-gray-300 shadow-lg overflow-y-auto">
        ...
        <div className="flex flex-col md:flex-row gap-4 mb-4">
          ...
          <div className="flex-1 flex flex-col">
            <label className="text-md font-semibold mb-2">Address</label>
            <AddressInput
            ...
            {address && (
              <p className="text-sm text-gray-700 mt-1">
                Seleccionado: <span className="font-medium">{address}</span>
              </p>
            )}
          </div>
          ...
          <div className="flex-1 flex flex-col space-y-4">
            ...
            <div className="flex flex-col w-full max-w-xl">
              ...
              <textarea value={tags} onChange={(e) => setTags(e.target.value)} placeholder="Separate tags with commas, e.g. Italian, Pizza, Vegan" className="w-full p-3 rounded-md min-h-[100px] resize-none" />
            </div>

            <div className="flex flex-col w-full max-w-xl">
              <label className="text-md font-semibold mb-2">Description</label>
              <textarea value={description} onChange={(e) => setDescription(e.target.value)} className="w-full p-3 rounded-md min-h-[100px] resize-none" />
            </div>
            ...
            <div className="flex items-center gap-2 mt-4">
              ...
              <label htmlFor="toggleTradingHours" className="text-md font-semibold">Show Trading Hours</label>
            </div>

            {showTradingHours && (
              <>
                <div>
                  <label className="text-md font-semibold mr-4 ">Mon-Fri</label>
                  <input type="time" value={monOpen} onChange={(e) => setMonOpen(e.target.value)} className="input"/>
                  <input type="time" value={monClose} onChange={(e) => setMonClose(e.target.value)} className="input"/>
                </div>
                <div>
                  <label className="text-md font-semibold mr-4">Saturday</label>
                  <input type="time" value={satOpen} onChange={(e) => setSatOpen(e.target.value)} className="input"/>
                  <input type="time" value={satClose} onChange={(e) => setSatClose(e.target.value)} className="input"/>
                </div>
                <div>
                  <label className="text-md font-semibold mr-4">Sunday</label>
                  <input type="time" value={sunOpen} onChange={(e) => setSunOpen(e.target.value)} className="input" />
                  <input type="time" value={sunClose} onChange={(e) => setSunClose(e.target.value)} className="input"/>
                </div>
              </>
            )}
            ...
            <div className="flex items-center gap-2 mt-4">
              ...
              <label htmlFor="toggleOrderingTimes" className="text-md font-semibold"> Show Ordering Times</label>
            </div>

            {showOrderingTimes && (
              <>
                <div>
                  <label className="text-md font-semibold mr-4">Pickup Time (min)</label>
                  <input type="number" value={pickup} onChange={(e) => setPickup(e.target.value)} className="input"/>
                </div>

                <div>
                  <label className="text-md font-semibold mr-4">Dine-in Time (min)</label>
                  <input type="number" value={dinein} onChange={(e) => setDinein(e.target.value)} className="input"/>
                </div>
              </>
            )}
          </div>
