...
export default function EditableCategories({ slug, refetchProducts }: EditableCategoriesProps) {
    ...
    const confirmDelete = async () => {
        ...
        try {
            const res = await fetch(`${process.env.NEXT_PUBLIC_API_URL}/${slug}/categories/${categoryToDelete.id}`, {
                method: "DELETE",
                headers: {
    ...
    return (
        <section className="p-4">
            <div className="flex gap-4 overflow-x-auto pb-4">
                {categories.map((cat) => (
                    <div key={cat.id} className="min-w-[240px] h-[100px] bg-default-100 rounded-lg p-3 flex flex-col justify-between">
            ...
            <AddCategoryModal open={isCategoryModalOpen} onClose={() => setIsCategoryModalOpen(false)} onSave={handleCreateCategory} />
