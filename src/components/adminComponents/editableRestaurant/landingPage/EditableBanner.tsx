...
export default function EditableBannerHero({ title, initialBanner }: EditableBannerCropProps) {
    ...
    return (
        <div className="relative w-full h-[33vh] sm:h-[40vh] md:h-[50vh] bg-gray-100 overflow-hidden">
            ...
            <h1 className="absolute z-20 inset-0 flex items-center justify-center text-white text-3xl md:text-5xl font-bold">{title}</h1>
            ...
            {imageSrc && (
                <div className="absolute inset-0 z-40 bg-white/90 flex flex-col items-center justify-center">
                    <div className="relative w-full h-[300px] bg-black/30">
                        <Cropper image={imageSrc} crop={crop} zoom={zoom} aspect={3 / 1} onCropChange={setCrop} onZoomChange={setZoom} onCropComplete={onCropComplete} />
                    </div>

                    <div className="w-full p-4 flex flex-col items-center gap-2 bg-white">
                        <label className="text-sm font-medium">Zoom</label>
                        <input type="range" min={1} max={3} step={0.1} value={zoom} onChange={(e) => setZoom(Number(e.target.value))} className="w-3/4" />
                        <div className="flex gap-4 mt-4">
                            <button onClick={handleSave} className="bg-sage-600 text-white px-4 py-2 rounded hover:bg-sage-800 cursor-pointer">
                                Save Image
                            </button>
                            <button onClick={() => setImageSrc(null)} className="bg-gray-400 text-white px-4 py-2 rounded hover:bg-sage-800 cursor-pointer">
                                Cancel
                            </button>
                        </div>
                    </div>
