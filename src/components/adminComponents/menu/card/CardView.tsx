...
import { IMenuCreateForm } from '../menuTypes/menuTypes'

const CardView = ({name, description, price, imageFile, detail, available, category}: IMenuCreateForm) => {
  return (
    <div className="flex justify-center p-4 w-full sm:w-20 md:w-96 lg:w-[22rem] m-auto">
      ...
      <div className="flex items-center rounded-2xl shadow-lg p-4 gap-4 hover:shadow-md hover:scale-105 transition-transform duration-200 w-full bg-white">
        ...
        {imageFile && (
          <img src={imageFile} alt={name} className="w-30 h-30 object-cover rounded-md"/>
        )}
      </div>
    </div>
  )
