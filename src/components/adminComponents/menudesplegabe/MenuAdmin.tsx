        ...
        {isOpen && (
          <div className='flex flex-col space-y-2 pt-2'>
            <Link href="/dashboard" onClick={handleLinkClick} className={linkClasses('/dashboard')}>
              {pathname === '/dashboard' && (<div className="absolute left-0 top-2 bottom-2 w-1 bg-black " />)}<HiOutlineHome /> Home
            </Link>
            <Link href="/dashboard/stores" onClick={handleLinkClick} className={linkClasses('/dashboard/stores')}>
              {pathname === '/dashboard/stores' && (<div className="absolute left-0 top-2 bottom-2 w-1 bg-black " />)}<HiOutlineBuildingStorefront /> Stores
            </Link>
            <Link href="/dashboard/orders" onClick={handleLinkClick} className={linkClasses('/dashboard/orders')}>
              {pathname === '/dashboard/orders' && (<div className="absolute left-0 top-2 bottom-2 w-1 bg-black " />)}<HiOutlineShoppingBag /> Orders
            </Link>
            <Link href="/dashboard/reports" onClick={handleLinkClick} className={linkClasses('/dashboard/reports')}>
              {pathname === '/dashboard/reports' && (<div className="absolute left-0 top-2 bottom-2 w-1 bg-black " />)}<HiOutlineChartBar /> Reports
            </Link>
            <Link href="/dashboard/menu/createproduct" onClick={handleLinkClick} className={linkClasses('/dashboard/menu')}>
              {pathname === '/dashboard/menu/createproduct' && (<div className="absolute left-0 top-2 bottom-2 w-1 bg-black " />)}<GiKnifeFork /> Menu
            </Link>
            <Link href="/dashboard/settings" onClick={handleLinkClick} className={linkClasses('/dashboard/settings')}>
              {pathname === '/dashboard/settings' && (<div className="absolute left-0 top-2 bottom-2 w-1 bg-black " />)}<HiOutlineCog6Tooth /> Settings
            </Link>
          </div>
        )}
      </div>
