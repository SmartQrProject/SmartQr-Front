    ...
    if (typeof window !== "undefined") {
      ...
      if (sessionRaw) {
        try {
          ...
          const extractedSlug =
            session.restaurant?.slug || session.payload?.restaurant?.slug;
  ...
  if (!slug || !token) {
    return (
      <p className="text-center mt-10 text-red-600">
        Unable to retrieve token or slug. Please check login.
      </p>
    );
  }
  ...
  return (
    <div className="px-4">
      <TableSeederForm
        ...
        urlback={urlback}
        token={token}
        onTablesUpdated={() => fetchTables(slug, token)}
      />
    
      <TableList
        slug={slug}
        tables={tables}
        onTableDeleted={() => fetchTables(slug, token)}
        onTableEdited={() => fetchTables(slug, token)}
      />
    </div>
  );
};
